...
public class GodCam extends Camera implements UpdateListener {
    float speed=2;
    Vector2f lastFrame;
    private final float boderWidth=20;//pixels
    private final float borderHeight=20;//pixels
    public GodCam(Vector3f position, Vector3f rotation, float fov, float viewDistance,float speed) {
    ...
    public void update(float deltaTime) {
        ...
        else if(mousePos.y>Display.getWidth()-borderHeight)
        {
            change.y=1f*deltaTime;
        }
       ...
       // this.transform.rotation.x+=change.y*(float)Math.cos(Math.toRadians(transform.rotation.y));
        //this.transform.rotation.z+=change.y*(float)Math.sin(Math.toRadians(transform.rotation.y));
